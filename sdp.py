...
def pack_sdp_pdu(pdu_id: str, payload: bytes, tid=None, plen=None):
    if tid is None:
...
def pack_seq8(payload: bytes) -> bytes:
    assert len(payload) < 0x100
    SDP_SEQ8 = 0x35
    return b''.join([c.to_bytes(1, "little") for c in (SDP_SEQ8, len(payload))]) + payload

def pack_uuid16(value: int) -> bytes:
    assert 0 <= value <= 0xffff
    SDP_UUID16 = 0x19
    return struct.pack('>BH', SDP_UUID16, value)

def pack_uuid32(value: int) -> bytes:
...
def pack_services(services: List[int]) -> bytes:
    return pack_seq8(b''.join(map(pack_uuid16, services)))

def pack_attribute(attribute: Union[Tuple[int, int], int]) -> bytes:
...
def pack_attributes(attributes: List[Union[Tuple[int, int], int]]) -> bytes:
    return pack_seq8(b''.join(map(pack_attribute, attributes)))

def pack_search_attr_request(services: List[int], attributes: List[Union[Tuple[int, int], int]], max_response_size=0xffff, cstate=b'') -> bytes:
    ...
    # State
    if len(cstate) > 0:
        payload += len(cstate).to_bytes(2, "little") + cstate
    else:
        payload += b"\x00"
...
def pack_search_request(uuid: int, max_replies = 0xffff, cstate = b'') -> bytes:
    payload = pack_seq8(pack_uuid16(uuid))
    # Max replies, in records (each one is uint32)
    payload += struct.pack('>H', max_replies)
    # State
    payload += len(cstate).to_bytes(1, "little") + cstate
...
def do_search_attr_request_full(socket: socket.socket, services: List[int], attributes: List[Union[Tuple[int, int], int]], max_response_size=0xffff):
    cstate = b''
    while True:
        #print("Packing")
